...
import { shieldCredClass } from '../pages/shieldCredClass';
...
test.describe('AllTool Page elements:  @Sanity',()=>{
    ...
    test.beforeEach(async ({browser}) => {
        const context = await shieldCredClass.setupContext(browser);
        page = await context.newPage();
        allToolP = new AllToolPage (page);
    });

    test('Navigate to AllTool Page and check if Header, Breadcrumb is present on it',async()=>{
    ...
    test('Verify Banner Section',async function () {
    ...
    test('Verify Shop by industry Section',async()=>{
    ...
    test('Verify Sector Menus Section',async()=>{
    ...
    test('Verify Auto speciality Card',async({})=>{
